...
const testRepoRootEnv = "KITUP_TEST_REPO_ROOT"

func TestGoldenCases(t *testing.T) {
	if os.Getenv(testRepoRootEnv) == "" {
		t.Skip("shared golden cases require KITUP_TEST_REPO_ROOT")
	}
	var file goldenFile
	readJSON(t, repoPathFromCase("testdata/cases/bundled-skill-install.json"), &file)
...
func runCase(t *testing.T, tc goldenCase, home, workspace string) {
	...
	case "resolve-install-targets":
		targets, _, _, err := ResolveInstallTargets(
			caseBaseOptions(tc, home, workspace),
			agentSelector(opts["agents"]),
		...
		for _, target := range targets {
			actual = append(actual, map[string]any{
				"hostIds": target.HostIDs, "skillName": target.SkillName, "targetDir": target.TargetDir,
			})
		}
		equal(t, actual, expandValue(tc.Expected["targets"], home, workspace))
...
func repoPathFromCase(path string) string {
	...
	return filepath.Join(os.Getenv(testRepoRootEnv), path)
}
